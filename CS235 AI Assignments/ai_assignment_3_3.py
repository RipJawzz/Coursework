...
class particle():
    ...
    def update(self):
        ...
        for i in range(len(self.vel)):
            v_new.append(
                self.vel[i] + c1 * random.randint(0, 1) * (self.pbest_pos[i] - self.pos[i]) + c2 * random.randint(0,
                                                                                                                  1) * (
                        gbest_pos[i] - self.pos[i]))
            self.pos[i] += v_new[i]
        self.vel = v_new.copy()
        # setting bounds for pos
        ...
        '''#ensuring pos contains values found in marks
...
# updating swarm in each iteration
