...
typedef struct {
  pthread_mutex_t *mut;//mutex for exclusive access of the process
  int writers;
  int readers;
  int waiting;
  pthread_cond_t *writeOK, *readOK;//thread condition variable for read and write process
} rwl;

//read write object's structure
//which stores data for read and write process
typedef struct {
  rwl *lock;
  int id;
  long delay;
...
//Manages the reader's status like reading, sleeping
//and stores data in the reader itself
void *reader (void *args){
  rwargs *a;
  int d;
  a = (rwargs *)args;
...
//Manages the writer's status like writing, sleeping
//and stores data in the reader itself
void *writer (void *args){
  rwargs *a;
  int i;
...
rwl *initlock (void){
  ...
  if (lock->writeOK == NULL) { free (lock->mut); free (lock);
    ...
    //initialization of mutex, cond, details of readers and writers in lock
...
//lock for reading
//includes management of details and execution of waiting and mutex unlock
void readlock (rwl *lock, int d){
